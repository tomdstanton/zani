...
impl Database {
    ...
    pub fn push(&mut self, name: &[u8], meta: SketchMeta, raw_dict: &[u8], level: i32) {
        ...
        unsafe {
            let cdict_ptr = zstd_sys::ZSTD_createCDict(
...
// ==========================================
    ...
    fn compile_sequence(name: Vec<u8>, seq_bytes: &[u8], level: i32) -> Option<(Vec<u8>, SketchMeta, Vec<u8>)> {
...
impl ZaniEngine {
    ...
    pub fn query_matrix_batched(
        ...
        (0..queries.len()).into_par_iter().for_each_with(tx, |sender, q_idx| {
                ...
                let mut seq_buffer = vec![unsafe { std::mem::zeroed::<zstd_sys::ZSTD_Sequence>() }; max_seqs];
                let mut comp_buf = vec![0u8; bound];
                for t_idx in 0..db.len() {
                    ...
                    // Math Calcs
                    let calibrated_c_y_given_x = c_y_given_x.saturating_sub(t_meta.compressed as usize);
                    let max_cx_cy = std::cmp::max(t_meta.baseline, q_meta.baseline) as f64;
                    let ncd = (max_cx_cy - (max_cx_cy - calibrated_c_y_given_x as f64)) / max_cx_cy;
                    ...
                    let ani = if aligned_length > 0 { nt_match as f64 / aligned_length as f64 } else { 0.0 };
                    let gani = nt_match as f64 / query_size as f64;
                    let tani = (nt_match as f64 * 2.0) / (query_size as f64 + t_meta.size as f64);
                    ...
                    // Push to SoA block
                    local_batch.push(
                        t_idx as u32, ani, gani, tani, cov, similarity, 
                        nt_match, nt_mismatch, num_alns, len_ratio
                    );
...
pub mod io {
    ...
    /// Consumes the MPSC channel stream and writes directly to disk.
    ...
    pub fn write_tsv(
        ...
        // 8MB Buffer for NVMe-saturating write speeds
        let mut writer = BufWriter::with_capacity(8 * 1024 * 1024, file);

        // Write the header exactly matching the SoA columns
        writeln!(
            writer,
            "query_id\ttarget_id\tani\tgani\ttani\tcov\tncd_similarity\tnt_match\tnt_mismatch\tnum_alns\tlen_ratio"
        )?;
